...
% hold on
% plot(XN(outliers_cluster1, 2), yN(outliers_cluster1), 'og');
% hold off;
...
% hold on;
% plot(XN(outliers_cluster3, 16), yN(outliers_cluster3), 'og');
% hold off;
...
% f3 = figure;
% edges = 0:50:20000;
% h1 = histogram(y(setdiff(idx_cluster1, outliers_cluster1)), edges);
% h1.FaceColor = [0.0 0.0 1.0];
% hold on;
% h2 = histogram(y(idx_cluster2), edges);
% h2.FaceColor = [1.0 0.0 0.0];
% hold on;
% h3 = histogram(y(setdiff(idx_cluster3, outliers_cluster3)), edges);
...
% for feature=1:size(X,2)
%     f = figure;
%     plot(X(setdiff(idx_cluster1, outliers_cluster1),feature),y(setdiff(idx_cluster1, outliers_cluster1)),'ob');
%     hold on;
%     plot(X(idx_cluster2,feature),y(idx_cluster2),'or');
%     hold on;
%     plot(X(setdiff(idx_cluster3, outliers_cluster3),feature),y(setdiff(idx_cluster3, outliers_cluster3)),'og');
%     hold off;
%     ylabel('y');
%     xlabel(sprintf('Values of feature %d', feature));
